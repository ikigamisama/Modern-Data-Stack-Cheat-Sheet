...
class Proportional:
    ...
    def __init__(self):
        st.set_page_config(
            page_title="Proportional Visualizations", layout="wide")
    ...
    def render_header(self):
        ...
        with col1:
            ...
            **When to Use:**
            ...
            - Showing overlapping relationships
            - Displaying relative market sizes
            - Illustrating scale differences
        ...
        st.markdown("""
        **Supported Charts:**
        - **Venn Diagram** – Proportional overlapping sets
        - **Rose Chart (Nightingale)** – Polar area chart for cyclic comparisons
        - **Marimekko Chart** – Variable-width stacked bar (market share + segment)
        - **Bubble Cloud** – Packed bubbles sized by value
        """)
    ...
    def render_chart(self, chart_type: str, scenario: str):
        ...
        if chart_type == "Venn Diagram":
            fig = self.create_venn(scenario)
        elif chart_type == "Rose Chart (Nightingale)":
            fig = self.create_rose_chart(scenario)
        elif chart_type == "Marimekko Chart":
            fig = self.create_marimekko(scenario)
        else:
            fig = self.create_bubble_cloud(scenario)
